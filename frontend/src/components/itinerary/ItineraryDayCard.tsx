...
export function ItineraryDayCard({ day, tripId }: ItineraryDayCardProps) {
  ...
  return (
    <div className="bg-cloud-50 border border-cloud-200 rounded-xl p-4">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold text-cloud-900">{formattedDate}</h3>
          {day.notes && (
      ...
      {isLoading ? (
        <div className="space-y-2">
          {[1, 2].map((i) => (
            <div key={i} className="animate-pulse flex items-center gap-3 p-3 bg-white rounded-lg border border-cloud-100">
              <div className="w-8 h-8 bg-cloud-200 rounded-full flex-shrink-0" />
              <div className="flex-1 space-y-1.5">
                <div className="h-3.5 bg-cloud-200 rounded w-2/3" />
                <div className="h-3 bg-cloud-100 rounded w-1/3" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="text-sm text-cloud-500 text-center py-8">
