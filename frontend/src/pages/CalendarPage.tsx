  ...
  return (
    <div className="animate-pulse space-y-6">
      <div className="flex items-center justify-between">
        <div className="h-8 bg-cloud-200 rounded w-32" />
        <div className="h-10 bg-cloud-200 rounded w-36" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {Array.from({ length: 12 }).map((_, i) => (
          <div key={i} className="h-48 bg-cloud-200 rounded-xl" />
        ))}
      </div>
    </div>
  )
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2 text-cloud-400 hover:text-cloud-600 hover:bg-cloud-100 rounded-lg transition-colors"
            aria-label="Previous year"
          >
            <ChevronLeft className="w-5 h-5" />
          </button>
          <h1 className="text-2xl font-bold text-cloud-900 tabular-nums">{year}</h1>
          <button
            onClick={() => setYear(y => y + 1)}
            className="p-2 text-cloud-400 hover:text-cloud-600 hover:bg-cloud-100 rounded-lg transition-colors"
